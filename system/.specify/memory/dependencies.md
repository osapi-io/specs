...
Every module path matches its repository location under
`github.com/osapi-io/`, and no repository uses a `replace` directive — each
dependency is a pinned version, so a local checkout is never silently
substituted for a release.
