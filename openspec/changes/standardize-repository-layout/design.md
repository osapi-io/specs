...
### Match the paths; pin only what something maintains

A repository provisions tools twice — `.mise.toml` locally, setup actions in
workflows — and the failure that matters is the two disagreeing, not either one
floating.

An earlier revision of this design required pinning both, on the reasoning that
a floating version makes the toolchain an untracked input to a check. That
reasoning assumed Dependabot would maintain the pins. It does not:
`dependabot.yml` covers `github-actions`, `gomod` and `npm`, and nothing watches
`.mise.toml`. A pin there is raised by hand or never.

So pinning `.mise.toml` while the workflow's setup action floats guarantees
divergence at the next release — the workflow moves, the local declaration does
not, and the gap shows up as a check failing on a file nobody edited. Pinning
both means two manual bumps with nothing to prompt them, which decays the same
way more slowly.

Where an update mechanism covers a version, both paths pin it and that mechanism
raises both. Where nothing does, both track the latest release and move
together. `just` is the second case: `mise` resolves `latest` to 1.58.0 and
`extractions/setup-just` installs 1.58.0, and they will keep agreeing without
anyone maintaining them.

What this accepts is that a release can reformat committed files and turn a
check red with no commit behind it. That is the cost of floating, and it is
smaller than the alternative: the failure appears locally and in continuous
integration at the same time, on the same file, instead of one side passing
while the other fails.

*Alternative considered:* pin both and bump by hand. Nothing prompts the bump,
so the pins drift from what is current until something forces the issue.

*Alternative considered:* teach Dependabot to watch `.mise.toml`. There is no
ecosystem for it, so this would be a bespoke job maintained for one file.
