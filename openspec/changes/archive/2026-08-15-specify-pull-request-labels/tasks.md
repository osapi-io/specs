...
- [x] 2a.1 `kind/agents` — `AGENTS.md`, `CLAUDE.md`, `.claude/**`
- [x] 2a.2 `kind/contributing` — `CONTRIBUTING.md`
- [x] 2a.3 `kind/policy` — `AI_POLICY.md`, `CODE_OF_CONDUCT.md`, `LICENSE`
- [x] 2a.4 `kind/toolchain` — `.mise.toml`, `.golangci.yml`, `.goreleaser.yaml`,
  `codecov.yml`, `.coverignore`, `.github/dependabot.yml`
- [x] 2a.5 Narrow `kind/docs` to `README.md` and `docs/**`, so it stops
...
- [x] 4.1 Confirm every label in a repository matches at least one file it holds
- [x] 4.2 Confirm every kind of file a repository holds has a label
- [x] 4.3 Confirm the same kind carries the same label name in every repository
- [x] 4.4 Confirm no two labels match the same file as a matter of course
- [x] 4.5 Confirm every tracked file kind is matched by some label, including
