...
- [ ] 1.6 `specs` — sync `repo-standards` into `openspec/specs/`. It is not
  there, so this change's delta has nothing to land on and the repositories
  already citing it point at a capability the corpus does not hold. Verified by
  `openspec/specs/repo-standards/spec.md` existing and
